...
public class DateUtils {
	...
	public static boolean judgeSettleDate(Date fromDate, Date toDate, int period) {
		...
		// 打印出来的是今天结算日期
		System.out.println("下次结算开始日期：" + cal_from.get(Calendar.YEAR) + "-"
				+ cal_from.get(Calendar.MONTH) + "-"
				+ cal_from.get(Calendar.DAY_OF_MONTH));
		// 打印出来的是上次结算日期
		System.out.println("上次结算日期：" + cal_to.get(Calendar.YEAR) + "-"
				+ cal_to.get(Calendar.MONTH) + "-"
				+ cal_to.get(Calendar.DAY_OF_MONTH));
		return cal_from.get(Calendar.DAY_OF_MONTH) == cal_to
				.get(Calendar.DAY_OF_MONTH) + period;
	}
	...
	public static boolean isOutNowTime(String time) {
		...
		if (isValidDate(time, "HH:mm:ss")) {
			Date nowDate = DateUtils.parseDate(
					DateUtils.formatDate("HH:mm:ss", new Date()), "HH:mm:ss");
	...
	/**
	 * 获得两个日期之间（包含）所有的日期集合 例如 传入 2015-01-01 2015-01-05 ，返回list
	 * 2015-01-01,2015-01-02,2015-01-03,2015-01-04,2015-01-05
	 * 
	 * @param startTime
	 *            开始时间，格式yyyy-MM-dd
	 * @param endTime
	 *            结束时间，格式yyyy-MM-dd fromat 返回对应格式
	 * @return
	 */
	public static List<String> getDaysByStartTimeToEndTime(String startTime,
		 ...
		 * return null;
		 */
		if (parseDate(startTime, DATE_PATTERN).getTime() > parseDate(endTime,
				DATE_PATTERN).getTime())
			return null;
		List<String> list = new ArrayList<String>();
		if (formatDate(format, parseDate(startTime, format)).equals(
				formatDate(format, parseDate(endTime, format)))) {
			if (!list
					.contains(formatDate(format, parseDate(startTime, format))))
				list.add(formatDate(format, parseDate(startTime, format)));
		}
		while (parseDate(startTime, DATE_PATTERN).getTime() <= parseDate(
				endTime, DATE_PATTERN).getTime()) {
			if (!list
					.contains(formatDate(format, parseDate(startTime, format))))
				list.add(formatDate(format, parseDate(startTime, format)));
			startTime = format(getAppointDate(
					parseDate(startTime, DATE_PATTERN), 1));
		}
		return list;
	}
	...
	// List<String> list = new ArrayList<String>();
	// list = getDaysByStartTimeToEndTime("2015-01-01",
	// "2015-05-03","yyyy-MM-dd");
